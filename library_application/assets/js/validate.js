function validateForm() {
  ...
  function checkNotEmpty() {
    console.log(document.validate_Adminform + " <-- validate adminform");
    ...
    if(document.validate_Adminform.genre.value == "" ){
        alertgenre();
        return false;
    }
    if(document.validate_Adminform.statys.value == "" ){
        alertstatus();
        return false;
    }
  ...
  function checkPattern() {
    ...
    var name_reg_ex = /^([A-Za-z]{1,30}[ \-]?[A-Za-z]{1,32}){1,32}$/; // needed for first/last name, title (genre&status)?
    var description_reg_ex = /^([A-Za-z.]{1,190}[ \-.]?){1,190}$/;
